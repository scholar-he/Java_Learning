...
注意事项：
	1. 强制类型转换一般不推荐使用，因为有可能发生精度的损失、数据溢出
	2. byte/short/char这三种类型都可以发生数学运算，例如加法“+”
	3. byte/short/char这三种类型在运算的时候，都会被首先提升成int类型，然后再计算
	4. boolean类型数据不能发生数据类型转换
*/
...
public class Demo02DataType{
	public static void main(String[] args){
		...
		char zifu1 = 'A'; //这是一个字符类型，里面是大写字母A
		System.out.println(zifu1 + 1); // 66,也就是大写字母A被当成65进行处理
		// 计算机的底层会用有个数字(二进制)来字符A, 就是65
